...
const setHeader = () => {
  ...
  for (const h of props.headers) {
    ...
    columns.value.push({
      name, label, align, sortable, field, style, classes, headerStyle, headerClasses, config: h
    });
  }
...
<template>
  <!-- {{ columns }} -->
  <q-table v-bind="$attrs" flat bordered
           :title="title"
          ...
          <template v-if="props.col?.config">
            ...
            <template v-if="props.col?.config.type === CrudListDataType.TEXT">
                            <span>
                                ...
                                <q-tooltip v-if="
                                    props.col?.config.options && props.col?.config.options.toolTip
                                ">
                                    {{
                                    props.value
                                  }}
            ...
            <template v-else-if="
                            props.col?.config.type === CrudListDataType.NUMBER_FORMAT
                        ">
              {{
            ...
            <template v-else-if="
                            props.col?.config.type === CrudListDataType.DATE_TIME
                        ">
              {{
            ...
            <template v-else-if="
                            props.col?.config.type === CrudListDataType.STATUS
                        ">
              <q-icon size="sm" :color="props.value
                                ? 'positive'
                                : 'grey-4'
                                "
                      :name="props.value ? (props.col?.config?.options?.trueIcon ? props.col?.config?.options?.trueIcon : biCheckCircle) : (props.col?.config?.options?.falseIcon ? props.col?.config?.options?.falseIcon : biCheckCircle)"></q-icon>
            </template>
            <template v-else-if="
                            props.col?.config.type === CrudListDataType.AVATAR
                        ">
              <q-avatar :rounded="props.col?.config.options.rounded"
                        :square="props.col?.config.options.square" :size="props.col?.config.options.size
                                    ? props.col?.config.options.size
                                    : '36px'
                                    ">
                <q-img v-if="props.value" :src="props.value" :alt="'item-img-' + props.rowIndex"
                       :ratio="1" />
            ...
            <template v-else-if="props.col?.config.type === CrudListDataType.IMAGE">
              <q-img v-if="props.value" :src="props.value" spinner - color=" white" class="bg-grey-8"
                     :ratio="4 / 3" :style="props.col?.config.options.size
            ...
            <template v-else-if="props.col?.config.type === CrudListDataType.ICON">
              <q-icon v-if="props.value" :name="props.value"></q-icon>
            </template>
          </template>
          <template v-else>
            {{ props.value }}
